...
void Game::ControlGameState() {
	...
	if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
		...
		case 0:{
			...
			if (board[pos].peice->id != peices::none && ((board[pos].peice->color) == peices::white)) {
				startPos = pos;
				currState = 1;
				isHighlighted = true;
				}
		...
		case 2:{
			...
			if (board[pos].peice->id != peices::none && ((board[pos].peice->color == peices::black))) {
				startPos = pos;
				currState = 3;
				isHighlighted = true;
			}
...
Vec2 Game::GetMousePos() {
	...
	return {static_cast<int>(mousePos.x / 100),static_cast<int>(mousePos.y/ 100)};
}

void Game::Draw() {
	DrawTexture(allTextures["Board"], 10, 10, WHITE);
	if (isHighlighted){ DrawRectangle( 100*startPos.x +10,
			100*startPos.y+10,100,100,Color{244, 236, 194,200});
	}
	DrawPeices();
	if (isHighlighted){
		MakeHighlight();
...
void Game::DrawPeices()
{
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			...
			if (currpeice != "") {
				DrawTexture(allTextures[currpeice], 20 + (j * 100), 20 + (i * 100), WHITE);
			}
...
void Game::MakeHighlight(){
	...
	for(Vec2 it :moves){
		DrawEllipse(100*it.x +60, 100*it.y+60,12.0f,12.0f,Color{127,127,127,127});		
	}
}
