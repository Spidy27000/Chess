...
std::vector<Vec2> mPeice::GetMoves(std::vector<std::string> directions, int color,Peice currBoard[][8]){
...
std::vector<Vec2> mPeice::GetMoves(std::vector<Vec2> directions, int color,Peice currBoard[][8]){
		std::vector<Vec2> validMoves = {};
		Vec2 temp = this->pos;
...
std::vector<Vec2> mPeice::GetMoveInDirection(Vec2 dir,int color ,Peice board[][8])  {
	std::vector<Vec2> validMove = {};
	Vec2 tempPos= this->pos  + dir;
	while ((tempPos.x <= 7 ) && (tempPos.x >= 0)&&
			(tempPos.y<= 7) && (tempPos.y >= 0))
	{
