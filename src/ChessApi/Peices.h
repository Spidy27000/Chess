...
public:
  ...
  std::vector<Vec2> GetNormalMoves(Peice currBoard[][8]) override {
    ...
    if (((tempPos.x >= 0) && (tempPos.x <= 7) && (tempPos.y >= 0) &&
         (tempPos.y <= 7)) &&
      ...
      if (((tempPos.x >= 0) && (tempPos.x <= 7) && (tempPos.y >= 0) &&
           (tempPos.y <= 7)) &&
          (!hasMoved) && (currBoard[tempPos.x][tempPos.y].peice->color == 0)) {
        validMove.push_back(tempPos);
      }
    }
    tempPos = this->pos;
...
public:
  NonePeice(Vec2 pos) : mPeice(pos, 0, peices::none) { this->id = 0; }
  ~NonePeice(){};
  std::vector<Vec2> GetNormalMoves(Peice currBoard[][8]) override { return {}; }
};
